...
function gpxToGeoJSON(gpxContent: string): any {
  try {
    const xml: any = parse(gpxContent);
...
// Routes
router.get("/api/routes", async (ctx: Context) => {
...
router.post("/api/routes", async (ctx: Context) => {
...
router.delete("/api/routes/:id", async (ctx: Context) => {
    // @ts-ignore
...
router.get("/api/routes/:id/download", async (ctx: Context) => {
  // @ts-ignore
  const id = ctx.params.id;
...
router.put("/api/routes/:id", async (ctx: Context) => {
    // @ts-ignore
    const id = ctx.params.id;
    const body = ctx.request.body();
...
while (!connected) {
    try {
        ...
        console.log("Failed to connect to DB, retrying in 5s...", e.message);
